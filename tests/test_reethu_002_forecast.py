...
Covers the forecasting-layer contracts without modifying production behavior.
"""
...
def _service_times(scenario):
    return {q.queue_id: q.avg_service_time_minutes for q in scenario.queues}


def _adequate_allocation(scenario):
    # Allocate the minimum valid staff to every canonical queue.  The
    # simulation contract only requires queue keys here; it does not require
    # this test to prove optimization quality.
...
def test_forecast_slot_count_matches_scenario():
    ...
    for name in SCENARIOS:
        ...
        assert all(len(values) == len(result.slots) for values in result.expected_arrivals.values())
...
def test_forecast_result_rejects_mismatched_arrival_lengths():
    """The current ForecastResult contract rejects queue lists with wrong lengths."""
    ...
    try:
        ...
        return
    raise AssertionError("ForecastResult must reject mismatched arrival-list lengths")

    ...
    <aside className="vortex-sidebar">
      ...
      <div className="sidebar-status-block">
        ...
        <div className={`status-pill-${serverStatus?.online ? 'online' : 'offline'}`}>
          ...
          <span className="status-text">{serverStatus?.online ? 'SYSTEM ONLINE' : (isLiveApi ? 'BACKEND OFFLINE' : 'LOCAL ENGINE')}</span>
        </div>
        <div className="api-connectivity-row">
          ...
          <span className={`api-state ${serverStatus?.online ? 'connected' : 'offline'}`}>
            {serverStatus?.online ? 'FASTAPI CONNECTED' : 'DETERMINISTIC LOCAL ENGINE'}
          </span>
        </div>

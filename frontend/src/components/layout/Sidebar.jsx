    ...
    <aside className="vortex-sidebar">
      ...
      <div className="sidebar-brand-block">
        ...
        <div className="brand-text-wrap">
          <h1 className="brand-title">
            VORTEX <span className="brand-accent">SICM</span>
          </h1>
          <div className="brand-sub">BANK OPERATIONS</div>
        </div>
      </div>

      {/* Navigation List — Exactly 6 Required Items */}
      ...
      <div className="sidebar-status-block">
        ...
        <div className="api-connectivity-row">
          ...
          <span className={`api-state ${serverStatus?.online ? 'connected' : 'offline'}`}>
            {serverStatus?.online ? 'CONNECTED' : 'OFFLINE'}
          </span>
          <button
            className="api-toggle-btn"
            onClick={onToggleApiMode}
            title="Toggle between FastAPI endpoint and deterministic engine"
          >
            [{isLiveApi ? 'FASTAPI' : 'MOCK'}]
          </button>
        </div>

...
export default function WhatIfSimulator({
  ...
  const utilization = whatIfMetrics?.avg_utilization ?? whatIfMetrics?.utilization;
    ...
    <section className="vortex-section what-if-section" id="what-if">
      ...
      <div className="vortex-card what-if-card">
        <div className="what-if-header-bar">
          <div className="simulation-notice-badge font-mono"><span>● EXPERIMENTAL SIMULATION ONLY</span></div>
        ...
        <div className="what-if-queues-grid">
          {queues.map(q => {
            ...
            const count = allocation[qid] ?? q.min_staff ?? 1;
              ...
              <div key={qid} className="what-if-queue-row">
                ...
                <div className="what-if-current-staff">
                  ...
                  <div className="stepper-wrap">
                    <button className="stepper-btn minus" onClick={() => handleStep(qid, -1)} disabled={count <= minStaff} aria-label={`Decrease ${q.name} staff`}>−</button>
                    <span className="stepper-val font-mono cyan">{count}</span>
                    <button className="stepper-btn plus" onClick={() => handleStep(qid, 1)} disabled={count >= maxStaff} aria-label={`Increase ${q.name} staff`}>+</button>
                  </div>
                </div>
              </div>
            );
        ...
        <div className="what-if-action-row">
          <button className="cmd-btn-primary full-width" onClick={handleRun} disabled={!isValidBudget || loading}>
            {loading ? <><span className="btn-spinner" /><span>SIMULATING QUEUE LOAD...</span></> : <span>RUN WHAT-IF</span>}
          </button>
        </div>

        {whatIfMetrics ? (
          <div className="what-if-results-box">
            <div className="results-box-title font-mono"><span>SIMULATION OUTCOME // EXPERIMENTAL TELEMETRY</span></div>
            <div className="results-metrics-grid">
              <div className="res-metric-cell">
                <span className="res-metric-label font-mono">AVG WAIT</span>
                <span className="res-metric-val font-mono green">{avgWait == null ? '—' : `${avgWait.toFixed(1)} MIN`}</span>
                <span className="res-sub font-mono">SIMULATED AVERAGE</span>
              </div>
              <div className="res-metric-cell">
                <span className="res-metric-label font-mono">P95 WAIT</span>
                <span className="res-metric-val font-mono cyan">{p95Wait == null ? '—' : `${p95Wait.toFixed(1)} MIN`}</span>
                <span className="res-sub font-mono">TAIL LATENCY</span>
              </div>
              <div className="res-metric-cell">
                <span className="res-metric-label font-mono">UTILIZATION</span>
                <span className="res-metric-val font-mono yellow">{utilPercent == null ? '—' : `${utilPercent}%`}</span>
                <span className="res-sub font-mono">SYSTEM CAPACITY</span>
              </div>
              <div className="res-metric-cell">
                <span className="res-metric-label font-mono">OVERLOADED SLOTS</span>
                <span className={`res-metric-val font-mono ${Number(overloadedSlots) > 0 ? 'critical' : 'green'}`}>
                  {overloadedSlots == null ? '—' : `${overloadedSlots} SLOTS`}
                </span>
          ...
          <div className="what-if-placeholder font-mono"><span>AWAITING INPUT — ADJUST STAFF AND CLICK [ RUN WHAT-IF ] TO EVALUATE</span></div>
        )}
      </div>
    </section>
  );
